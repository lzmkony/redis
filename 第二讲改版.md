...
#define ZIP_INT_MASK 0x30    0000 0011
...
1.找出上个entry的具体长度值，与需要的空间
2.找出本entry编码类型，与需要存储的空间
3.判断下个节点是否会因为本节点长度，导致变大
4.按上面的信息重新分配空间
5.把具体的值存进去，修改ziplist长度，尾节点长度，节点个数
...
头部insert
1.检查头部ziplist是否可以insert
2.如果可以，则直接走ziplist的insert
3.不可以，则新建quicklistNode
4.把新建的quicklistNode在quicklist的链表头部insert
...
insert
1.判断新insert的数据是否编码小于当前编码
2.如不小于，则头前intset全部扩展
3.找到要insert的指定位置，如果值已经存在，则返回
4.向后平移一位insert后后面数据，把当前值insert进去
5.修改intset长度
